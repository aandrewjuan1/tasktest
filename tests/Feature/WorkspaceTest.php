...
test('can search for tasks', function () {
    Task::factory()->create([
        'user_id' => $this->user->id,
...
test('can filter tasks by status', function () {
    ...
    Livewire::test('workspace.show-items')
        ->set('statusFilter', 'to_do')
        ->assertSee('Todo Task')
        ->assertDontSee('Done Task');
});

test('can filter by item type', function () {
    Task::factory()->create([
        'user_id' => $this->user->id,
        'title' => 'Task Item',
    ]);
...
test('can filter tasks by tags', function () {
    $tag1 = \App\Models\Tag::factory()->create(['name' => 'Urgent']);
    $tag2 = \App\Models\Tag::factory()->create(['name' => 'Important']);
    ...
    Livewire::test('workspace.show-items')
        ...
        ->assertSee('Tagged Task 1')
        ->assertDontSee('Tagged Task 2');
});

test('can filter tasks by project', function () {
    $project = Project::factory()->create([
        'user_id' => $this->user->id,
        'name' => 'Test Project',
    ]);
...
test('can filter by date range today', function () {
    Task::factory()->create([
        'user_id' => $this->user->id,
        'title' => 'Today Task',
        'end_date' => now(),
